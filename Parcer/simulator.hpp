...
void start_simulation(const Context& ctx) {
    if constexpr (Rows == kDynamicExtent && Columns == kDynamicExtent) {
        auto LS = LiquidSimDynamic<10, PElementType, VelocityElementType, VelocityFlowElementType>(ctx.field.size(), ctx.field.front().size() - 1, ctx.field);//field, rho, p, p_old, velocity, velocity_flow, last_use, UT);
        LS.save("FIXED_DYN");
        LS.simulate();
    } else {
        auto LS = LiquidSim<Rows, Columns + 1, 10, PElementType,VelocityElementType, VelocityFlowElementType>(ctx.field);
        LS.save("FIXED_ST");
        LS.simulate();
    }
}
